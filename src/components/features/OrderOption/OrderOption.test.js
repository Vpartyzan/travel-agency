    ...
    switch (type) {
      ...
      case 'icons': {
        it('contains icons', () => {
          ...
          const emptyDivIcon = div.find('Icon[name="times-circle"]').length;
          expect(emptyDivIcon).toBe(1);

          const divIcons = renderedSubcomponent.find('.icon');
          expect(divIcons.length).toBe(mockProps.values.length + 1);
          expect(divIcons.at(1).find('Icon').prop('name')).toBe(mockProps.values[0].icon);
          expect(divIcons.at(2).find('Icon').prop('name')).toBe(mockProps.values[1].icon);
        });
